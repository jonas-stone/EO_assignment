function [x_opt_real, fval] = main()
    ...
    %% ========================
    %  INITIAL POINT & BOUNDS (5 VARIABLES)
    %  ========================
    % Variables: [c_root, c_tip, alpha, velocity, twist]
    semi_span = 7.5;
    x_0 = [1.5,  0.75,  3.0, 27.78, 0]; 
    lb  = [0.40,  0.20, -2.0, 25, -5]; 
    ub  = [2.0,  0.80,  7.2, 55.55, 0];  
    
    x_scale = [1.5, 0.75, 5.0, 30, 2.0];
    ...
    function update_cache(x_bar)
        ...
        [W_target, W_wing_tmp, ~] = estimate_weight2(aircraft_tmp, aero_tmp, V);
        ...
        % --- CALCULATE ACTUAL STALL SPEED ---
        CL_max_airfoil = 1.6;
        % V = sqrt( W / (0.5 * rho * S * CLmax) )
        V_stall_mps = sqrt(W_target / (0.5 * aero_tmp.rho * aircraft_tmp.S * CL_max_airfoil));
        cache.V_stall = V_stall_mps * 3.6; % Convert to km/h and store in cache
        % ------------------------------------
        ...
        try
            ...
            fprintf('L/D = %6.2f | Err = %+6.2f%% | Stress = %6.1f MPa | V_stall = %5.1f km/h\n', ...
                    LD_raw, err_pct, stress_tmp, cache.V_stall);
        catch
            cache.LD = -1e6; % Objective penalty
            cache.L  = 1e9;  % Constraint penalty
            cache.stress = 1e9;
    ...
    function [c, ceq] = nonlcon_fun(x_bar)
        ...
        if cache.L > 1e8 
            ...
            c_root = x_bar(1) * x_scale(1);
            c_tip  = x_bar(2) * x_scale(2);
            twist  = x_bar(5) * x_scale(5); % Note: Fixed from x_scale(6) to prevent index error
            V      = x_bar(4) * x_scale(4);
    ...
    function stop = my_output_fun(x, optimValues, state)
       ...
       if isequal(state, 'iter')
            ...
            fprintf('Current Geometry : Root = %5.3f | Tip = %5.3f | Semi-Span = %5.2f | Twist = %5.2f | Alpha = %5.2f | V = %5.1f\n', ...
                    x_real(1), x_real(2), semi_span, x_real(5), x_real(3), x_real(4));
            ...
            fprintf('Stall Speed      : %6.1f km/h (Limit: 80.0 km/h)\n', cache.V_stall);
