function [x_opt_real, fval] = main()
    ...
    x_0 = [1.5,  0.75,  2.0, 27.78, 7.5, 0]; 
    lb  = [1.0,  0.30, -2.0, 25.00, 5.0, -5]; 
    ub  = [2.0,  0.80,  7.2, 55.55, 30.0, 5];  
    ...
    function update_cache(x_bar)
        ...
        if Max_Lift_Possible < W_target
            ...
            fprintf('  [CACHE] Alpha=%5.2f | Root=%5.2f | Tip=%5.2f | Span=%5.2f | Twist=%5.2f | V=%5.1f ... INSTANT FAIL (Area/Speed too low)\n', ...
                    alpha, c_root, c_tip, b2, twist, V);
            return; % Exit instantly
        end
        % ---------------------------------------
        
        fprintf('  [CACHE] Alpha=%5.2f | Root=%5.2f | Tip=%5.2f | Span=%5.2f | Twist=%5.2f | V=%5.1f ... ', ...
                alpha, c_root, c_tip, b2, twist, V);
                
        try
            % Run the heavy physics model
            [LD_raw, L_tmp, ~] = run_model([b2, c_root, c_tip, twist, V, alpha]);
